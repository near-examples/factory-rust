...
const DEFAULT_CONTRACT: &[u8] =
    include_bytes!("../src/status-message-contract/status_message.wasm");
...
async fn test_deploy_global_contract() -> anyhow::Result<()> {
    ...
    let status_code = DEFAULT_CONTRACT.to_vec();
    ...
    assert!(
        res.is_success(),
        "Failed to deploy global contract: {res:?}"
    );
    ...
    assert!(
        stored_hash.is_some(),
        "Global contract hash should be stored"
    );
    ...
    let contracts_list = factory_contract
        ...
        .view()
        .await?
        .json::<Vec<(String, Base58CryptoHash, AccountId)>>()?;
...
async fn test_use_global_contract_by_hash() -> anyhow::Result<()> {
    ...
    assert!(
        res.is_success(),
        "Failed to use global contract by hash: {:?}",
        res.outcome()
    );
...
async fn test_use_global_contract_by_account() -> anyhow::Result<()> {
    ...
    let global_account_id: AccountId =
        format!("status_message.{}", factory_contract.id()).parse()?;
    ...
    assert!(
        res.is_success(),
        "Failed to deploy global contract by account: {res:?}"
    );
    ...
    assert!(
        res.is_success(),
        "Failed to use global contract by account: {res:?}"
    );
...
async fn test_global_contract_edge_cases() -> anyhow::Result<()> {
    ...
    assert!(
        empty_list.is_empty(),
        "Should return empty list when no contracts deployed"
    );
    ...
    assert!(
        res.is_failure(),
        "Not failed to use global contract by account: {res:?}"
    );
